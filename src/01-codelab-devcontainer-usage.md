...
- `src` ディレクトリの中の、拡張子が `md` のファイルと `img` ディレクトリを削除します。
    - `src/index.html`, `src/custom.css` は使用するため残してください。
- `README.md` を削除します。
...
作成した codelab の各ページではカスタムスタイルを読み込むようになっています。`src/custom.css` を編集することで、カスタムスタイルを設定することができます。

たとえば、`src/custom.css` に以下のように記述することで、各ページの `###` で書いた見出しが太字になります。

```css
google-codelab-step .instructions h2:not(.step-title) {
